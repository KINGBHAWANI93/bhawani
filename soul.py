...
def send_not_approved_message(chat_id):
    bot.send_message(chat_id, "*pehle key to krid fir message kr @BAD_BOY_ROWD *", parse_mode='Markdown')
...
def approve_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*jayda bol rha h bsdk nikal ja ise batt kr @BAD_BOY_ROWD*", parse_mode='Markdown')
        return
...
def disapprove_user(message):
    if not is_user_admin(message.from_user.id, message.chat.id):
        bot.send_message(message.chat.id, "*marvali gand ise batt kr madrachod @BAD_BOY_ROWD*", parse_mode='Markdown')
        return
